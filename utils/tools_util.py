...
class Tools:
    def __init__(self, tools_file=None, tools=None, load_raw=None):
        ...
        if tools_file is not None:
            ...
            with open(load_raw) as f:
                self.tools_raw = json.load(f)
            self.tools = None
        self.tools_api = None
    ...
    def load_tools_via_api(self, tools):
        # Any format is fine
        # self.tools_api = tools
        formated_tools = {}
        # for tool in tools:
        #     args = []
        #     for arg in tool.get('schemaElements'):
        #         args.append(
        #             f"""- name: {arg.get('name')}
        ...
        #     formated_tools[tool.get('name', 'dummy')] = f"""{tool.get('name', 'dummy')}:
        # tool: {tool.get('name', 'dummy')}
        ...
        for tool in tools:
            args = []
            for arg in tool.get('schemaElements'):
                args.append({"name": arg.get('name'), "type": arg.get('data_type'), "description": arg.get('description'), "example": arg.get('example')})
            formated_tools[tool.get('name', 'dummy')] = {"tool": tool.get('name', 'dummy'), "description": tool.get('description'), "arguments": args}
        
        self.tools = formated_tools
    
    def get_frontend_tools(self):
        ## can you write a code which can do the inverse of the above function
        # return self.tools_api
        front_tools = []
        for tool_name, tool in self.tools.items():
            args = []
            for arg in tool.get('arguments', []):
                args.append({"name": arg.get('name'), "data_type": arg.get('type'), "description": arg.get('description'), "example": arg.get('example')})
            front_tools.append({"name": tool_name, "description": tool.get('description'), "schemaElements": args})
        return front_tools
