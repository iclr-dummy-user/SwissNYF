...
class ReverseChain(BasePipeline):
    ...
    def set_raw_tools(self, tool_defs):
        for tool_def in tool_defs:
            idx = tool_def.find("def ") + 4
            tool_name = tool_def[idx:].split("(")[0]
            formated_tool_def = tool_def.replace("@update_traverse_dict", "")
    ...
    def fetch_plan_w_goal(self, query):
        ...
        res = self.llm.complete(fetch_plan_w_goals_prompt) #, stream=True)
        ...
        if idx>=0:
            ...
            return final_api_list, res.text
        else:
    ...
    def fetch_plan(self, query):
        ...
        res = self.llm.complete(first_system_prompt)#, stream=True)
        final_api = res.text
        return [final_api]
    ...
    def build_tree_for_f_api(self, final_api, query):
        ...
        while len(tools_req) != 0:
            ...
            available_tools = [t for t in self.all_tools]
            ...
            res = self.llm.complete(arg_retreiver)#, stream=True)
            res = res.text
    ...
    def query(self, query) -> ActionTree:
        final_apis, final_apis_n_goals = self.fetch_plan_w_goal(query)
        yield final_apis_n_goals
        trees = []
    ...
    def merge_trees(self, query, trees):
        ...
        merged_tree = self.llm.complete(self.MERGE_TREES_PROMPT.format(query, parsed_tree, [tool.metadata.name for tool in self.all_tools])).text #,stream=True
        return merged_tree
